...
void VirtualTableCommunicator::set_human_stick(
    const messages::Stick::ConstPtr &msg) {
  human_stick = *msg;
  emit refresh_gui();
}

void VirtualTableCommunicator::set_ai_stick(
    const messages::Stick::ConstPtr &msg) {
  ai_stick = *msg;
  emit refresh_gui();
}

const messages::Ball &VirtualTableCommunicator::get_ball() { return ball; }
