...
// the rate at which all stepper motor accelerate (decrement size for
// pulseDelayAngular and pulseDelayLinear)
#define accelStep 25
...
// rod 1 callbacks for desired angular and linear positions as well as desired
// angular velocity
...
// rod 2 callbacks for desired angular and linear positions as well as desired
// angular velocity
...
void currLinearPosOne(std_msgs::Int32 &msg) { currPosLinearOne = msg.data; }

void currAngularPosOne(std_msgs::Float32 &msg) { currPosAngularOne = msg.data; }

void currAngularDirOne(std_msgs::Int8 &msg) { currDirAngularOne = msg.data; }

// rod 2 callbacks for current angle, direction, and linear positions
void currLinearPosTwo(std_msgs::Int32 &msg) { currPosLinearTwo = msg.data; }

void currAngularPosTwo(std_msgs::Float32 &msg) { currPosAngularTwo = msg.data; }

void currAngularDirTwo(std_msgs::Int8 &msg) { currDirAngularTwo = msg.data; }

// linear motion desired position rod 1 callback
ros::Subscriber<std_msgs::Int32>
    setLinearPositionOneCallBack("desiredLinearPositionOne",
                                 &setLinearPositionOne);

// desired position angular rod 1 callback
ros::Subscriber<std_msgs::Int32>
    setAngularPositionOneCallBack("desiredAngularPositionOne",
                                  &setAngularPositionOne);

// desired velocity angular rod 1 callback
ros::Subscriber<std_msgs::Int8>
    setAngularVelocityOneCallBack("desiredAngularVelocityOne",
                                  &setAngularVelocityOne);

// linear motion desired position rod 2 callback
ros::Subscriber<std_msgs::Int32>
    setLinearPositionTwoCallBack("desiredLinearPositionTwo",
                                 &setLinearPositionTwo);

// desired position angular rod 2 callback
ros::Subscriber<std_msgs::Int32>
    setAngularPositionTwoCallBack("desiredAngularPositionTwo",
                                  &setAngularPositionTwo);

// desired velocity angular rod 2 callback
ros::Subscriber<std_msgs::Int8>
    setAngularVelocityTwoCallBack("desiredAngularVelocityTwo",
                                  &setAngularVelocityTwo);

// current position linear rod 1 callback
ros::Subscriber<std_msgs::Int32>
    currLinearPosOneCallBack("currLinearPositionOne", &currLinearPosOne);

// current position angular  rod 1 callback
ros::Subscriber<std_msgs::Int32>
    currAngularPosOneCallBack("currAngularPositionOne", &currAngularPosOne);

// current direction angular  rod 1 callback
ros::Subscriber<std_msgs::Int8>
    currAngularDirOneCallBack("currAngularDirectionOne", &currAngularDirOne);

// current position linear rod 2 callback
ros::Subscriber<std_msgs::Int32>
    currLinearPosTwoCallBack("currLinearPositionTwo", &currLinearPosTwo);

// current position angular  rod 2 callback
ros::Subscriber<std_msgs::Int32>
    currAngularPosTwoCallBack("currAngularPositionTwo", &currAngularPosTwo);

// current direction angular  rod 2 callback
ros::Subscriber<std_msgs::Int8>
    currAngularDirTwoCallBack("currAngularDirectionTwo", &currAngularDirTwo);

// takes desired_pos as input (int type, mm units) and currPosLinear (int type
// in mm) converts desired position in mm to desired position in steps and
// performs move accordingly looks to see that the target position is within +/-
// 3 mm, otherwise adjusts
...
void haltLinearOne() { digitalWrite(stepPinLinearOne, LOW); }
...
void haltLinearTwo() { digitalWrite(stepPinLinearTwo, LOW); }
...
void positionControlAngularOne(int desiredPosAngularOne, int currPosAngularOne,
                               int currDirAngularOne) {
...
void haltAngularOne() { digitalWrite(stepPinAngularOne, LOW); }

// add the option for three different speeds
// takes desired position in degrees, and current position in degrees
// has threshold of +/- thresholdAngular degrees margin of error
void positionControlAngularTwo(int desiredPosAngularTwo, int currPosAngularTwo,
                               int currDirAngularTwo) {
...
void haltAngularTwo() { digitalWrite(stepPinAngularTwo, LOW); }
...
void loop() {
  ...
  // have to be getting feedback from linear closed loop system before actually
  // doing anything Linear position controller for rod 1
  if (currPosLinearOne != 0 && desiredPosLinearOne != 0) {
    positionControlLinearOne(desiredPosLinearOne, currPosLinearOne);
  }

  // have to be getting feedback from linear closed loop system before actually
  // doing anything Linear position controller for rod 2
  if (currPosLinearTwo != 0 && desiredPosLinearTwo != 0) {
    positionControlLinearTwo(desiredPosLinearTwo, currPosLinearTwo);
  }
  // have to be getting feedback from angular closed loop system before actually
  // doing anything Linear position controller for rod 1
  if (currPosAngularOne != 0 && desiredPosAngularOne != 0) {
    positionControlAngularOne(desiredPosAngularOne, currPosAngularOne,
                              currDirAngularOne);
  }
  ...
  if (currPosAngularTwo != 0 && desiredPosAngularTwo != 0) {
    positionControlAngularTwo(desiredPosAngularTwo, currPosAngularTwo,
                              currDirAngularTwo);
  }
